...
def main():
    ...
    constraints_of_action_generation = [
        ...
        "forall w in D2 . a-goal-on(w,2) and a-goal-holding and not holding and docked(4) and on(w,4) implies A(w)",
        "forall w in D2 . a-goal-on(w,2) and a-goal-holding and not holding and docked(3) and on(w,3) implies A(w)",
        ...
        #From P4 to P2
        "a-goal-at(2) and docked(4) and holding and assigned(2) implies B(4)",
        # From P3 to P2
        "a-goal-at(2) and docked(3) and holding and assigned(2) implies B(3)",
        #Request location permission for P2.
        "exists x. a-goal-at(2) and docked(x) and holding implies S(2)",
        #AMR goes from P1 to P3 or P4, from P6, P7, P8 to P3 or P4, from P2 to P5, from P5 to P6, P7,P8
        "forall p in D2 . a-goal-at(p) and not holding and not equal(p,2) and assigned(p) implies C(p)",
        "forall p in D2 . a-goal-at(p) and not holding and not equal(p,2) implies S(p)",
        "forall w. a-goal-on(w,2) and at(2) and docked(2) and holding implies D(w,2)",
    ...
    event_processing = [
        ...
        "exists w. a-goal-on(w,2) and not holding implies adopt holding",
    ...
    action_specification = {
        "pickup": "forall w,p in D2 . pickup(w) and not holding and on(w,p) implies holding and not on(w,p)",
        ...
        "dropoff": "forall w. dropoff(w) and holding implies on(w,2) and not holding",
    ...
    # Self-contained sensor traces for the three-agent story (do NOT reuse the
    # shared MG_0*.txt, which other demos depend on). One trace set per test case:
    #   1 = error-free (all three deliver), 2 = non-fatal error (A3 hits E1 and
    #   changes its goal), 3 = fatal error (A3 hits E4, crashes, goal redistributed).
    ...
    if test == '1':
        story_files = {"A1": "STORY_A1_ef.txt", "A2": "STORY_A2.txt", "A3": "STORY_A3_ef.txt"}
    elif test == '2':
        story_files = {"A1": "STORY_A1_ef.txt", "A2": "STORY_A2.txt", "A3": "STORY_A3_nf.txt"}
    else:
        story_files = {"A1": "STORY_A1.txt", "A2": "STORY_A2.txt", "A3": "STORY_A3.txt"}
