...
# --- Performance infrastructure (ported from the vGOAL model-checking framework,
# TS_Generation.py; behaviour-preserving) -----------------------------------
#
# Fast deep-copy for the plain data structures used throughout this module
# (nested dict / list / tuple with immutable leaves, no reference cycles).
# Produces a fully independent copy identical in value to copy.deepcopy, but
# without deepcopy's memo/_keep_alive bookkeeping. Any unrecognised type falls
# back to the standard library, so it is always correct.
_std_deepcopy = copy.deepcopy


def _fast_deepcopy(obj):
    t = type(obj)
    # str leaves are inlined in the comprehensions below (they are the most common
    # leaf: predicate names and argument values), avoiding a recursive call each.
    if t is dict:
        return {k: (v if type(v) is str else _fast_deepcopy(v)) for k, v in obj.items()}
    if t is list:
        return [v if type(v) is str else _fast_deepcopy(v) for v in obj]
    if t is tuple:
        return tuple(v if type(v) is str else _fast_deepcopy(v) for v in obj)
    if t is str or t is int or t is float or t is bool or t is bytes \
            or obj is None or t is frozenset:
        return obj
    return _std_deepcopy(obj)


# --- Memoization for state-independent logical instantiation ---------------
# Rule instantiation depends only on constant inputs (knowledge base / action
# specification + domain + constants) but was recomputed for every reasoning
# cycle, doing a large amount of redundant deep-copying. These functions are
# pure, so we cache their result by argument *content* and hand back an
# independent deep copy on every call. Callers therefore observe exactly the
# same behaviour as before (they always received a fresh structure), only far
# fewer times.
_MISS = object()
_MEMO_REGISTRY = []


def _memoize_pure(func):
    cache = {}
    tokens = {}          # id(arg) -> (arg, repr(arg)); keeps args alive so ids

    def tok(o):          # are not reused, and repr is computed once per object
        e = tokens.get(id(o))
    ...
    wrapper._clear = lambda: (cache.clear(), tokens.clear())
    _MEMO_REGISTRY.append(wrapper)
    return wrapper


def _clear_memoization_caches():
    for w in _MEMO_REGISTRY:
        w._clear()
...
def transform_vGOAL_to_CNF(rules, atoms, generated_atoms, keywords):
    ...
    for r in translated_rules:
        ...
        if derived_atom in translated_generated_atoms:
            flag = True
        # Empty body ("implies h"): h is unconditionally derived (a fact / axiom).
        # Without this, the loop below never runs (pos==0) and the atom's minimal-
        # support list stays empty, producing an empty CNF clause -> spurious UNSAT.
        # Treat it like the 'True' case: assert the fact and give it a tautology
        # justification so the minimality clause is satisfiable.
        if pos == 0:
            clause = [derived_atom]
            if clause not in CNF_Clauses:
                CNF_Clauses.append(clause)
            if flag:
                minimal_Generation_dict[derived_atom].extend([count, -count])
        while i < pos:
            if r[i] not in keywords:
                clause = [-r[i], derived_atom]
                CNF_Clauses.append(clause)
                if flag:
                    minimal_Generation_dict[derived_atom].append(r[i])
            elif r[i]=='True':
                clause=[derived_atom]
                if clause not in CNF_Clauses:
                    CNF_Clauses.append(clause)
                ...
                CNF_Clauses.append(clause)
                if flag:
                    minimal_Generation_dict[derived_atom].append(-r[i+1])
                i=i+1
            i = i + 1
    for key in minimal_Generation_dict:
        CNF_Clauses.append(minimal_Generation_dict[key])
    return CNF_Clauses
...
# SAT-based cross-check of the least-fixpoint derivation (Def. 2 correctness).
# The CNF built by transform_vGOAL_to_CNF from the knowledge-base rules + the
# LFP-derived atoms is satisfiable exactly when the derived model is a correct
# minimal model; an UNSAT result (S is False) means the least_fixpoint output
# disagrees with the SAT encoding. Reports on failure rather than raising, so one
# spurious mismatch cannot abort a whole run; flip the print to
# `raise Exception(...)` for hard-fail verification.
_SAT_FAIL_COUNT = 0


def _sat_report(S, where):
    global _SAT_FAIL_COUNT
    if not S:
        _SAT_FAIL_COUNT += 1
        print("[SAT-CHECK FAILED] " + where + ": LFP derivation is not SAT-consistent")
    return S


_SKIP_SAT = False  # set True by the interpreter in story/demo (sensor_files) mode


def _sat_verify(rules_rep, atoms_rep, generated_rep, where):
    # Best-effort SAT cross-check: build the CNF, solve, and report an UNSAT.
    # The encoding does not cover every rule shape (e.g. composite communication
    # message strings), so a build/solve failure is treated as "not verifiable
    # here" and must never crash the interpreter — verification is an add-on.
    if _SKIP_SAT:
        return None
    try:
        M = transform_vGOAL_to_CNF(rules_rep, atoms_rep, generated_rep, keywords)
        S = SAT_Check(M)
    except Exception:
        return None
    return _sat_report(S, where)
...
def process_beliefs(beliefs, constants):
    processed = []
    for i in beliefs:
...
def process_belief_list(belief_list, constants):
    ...
    for i in belief_list:
        processed.append(process_beliefs(i, constants))
    return processed
        ...
        if predicate not in keywords:
            if var in predicate["variables"]:
                for value in domain:
                    ...
                    predicate_copy = _fast_deepcopy(predicate)
...
# State-independent (output depends only on the rule L + domain + constants and
# does not mutate its inputs), so memoized by argument content — verified
# byte-identical to the un-memoized version by the regression oracles.
@_memoize_pure
def universal_variable_instantiation(L, domain, constants):
    ...
    rules = []
    rules_copy = [_fast_deepcopy(rule)]
...
def separate_rules(L, domain, constants):
    ...
    for rule in L:
        ...
        processed_rules_copy = _fast_deepcopy(processed_rules)
...
def predicate_existential_variables_instantiation(atoms, predicate, constants):
    ...
    if not predicate["list_contain"]:
        predicate_copy = _fast_deepcopy(predicate)
        for atom in atoms:
            # Arity guard: only unify with atoms of the same argument count.
            # A same-name atom of a different arity (e.g. a nullary sensor
            # `holding` vs a rule predicate `holding(w)`) is a different predicate
            # and must not unify; without this the index accesses below raise
            # IndexError on the shorter list.
            if len(atom["values_in_non_list"]) != len(predicate_copy["values_in_non_list"]):
                continue
            flag = True
...
def existential_variable_rule_instantiation(existential_rule, atoms, constants):
    ...
    while i < len(instantiated_rules):
        rule = _fast_deepcopy(instantiated_rules[i])
        rule_copy = _fast_deepcopy(rule)
        count = 0
        flag = True
        temp = []
            ...
            if predicate not in keywords:
                if predicate["variables"] != []:
                    ...
                    if substitution != []:
                        flag = False
                        for sub in substitution:
                            rule_store = _fast_deepcopy(rule)
...
def pattern_match_at_left_rule(rule, atoms):
    pos = find_position_in_list(rule, 'implies')[0]
    i = 0
    flag = True
...
def least_fixpoint(atoms, fully_instantiated, partial_instantiated, constants):
    ...
    # If there are at least one rule to be evaluate, start the derivation process
        ...
        while signal:
            ...
            interpreted = []
            for rule in to_be_match:
                interpreted.append(pattern_match_at_left_rule(rule, atoms))
...
def state_property_generation(belief_base, knowledge_base, domain, constants):
    ...
    partial_instantiated_copy=_fast_deepcopy(partial_instantiated)
    ...
    atoms = least_fixpoint(atoms_current, fully_instantiated, partial_instantiated, constants)
    ...
    # SAT cross-check of the least-fixpoint result (the actual LFP correctness check).
    _sat_verify(F_rep, belief_base_rep, A_rep, "state_property_generation")
...
def action_constraints_analysis(action_constraints, atoms_current_state, atoms_goal_state, domain, constants):
    ...
    constraints = []
    All_Act_Cons_Name = []
    # Default so the return does not raise UnboundLocalError when a constraint
    # expands to no instantiated rules (empty instantiated_constraints loop).
    fully_instantiated_rep = []
    ...
    for constraint in instantiated_constraints:
        ...
        for rule in fully_instantiated:
            new_rule = pattern_match_at_left_rule(rule, atoms_current_state)
            enabled = derivation_at_right_rule(new_rule)
            if enabled != []:
                enabled_rep=transform_to_normalform(enabled[0])
                enabled_condition.append(enabled)
                _sat_verify(fully_instantiated_rep, atoms_current_rep, [enabled_rep], "action_constraints_analysis")
        ...
        if current_action_constraint != []:
            ...
            if current_action_constraint in action_constraints:
                ...
                for rule in instanitated_rule:
                    new_rule = pattern_match_at_left_rule(rule, atom_current_state)
                ...
                if substitution != []:
                    new_operation_rules = []
                    for sub in substitution:
                        operation_rule_copy = _fast_deepcopy(operation_rule)
                        new_rule = []
                    ...
                    for rule in new_operation_rules:
                        ...
                        for rule in instanitated_rule:
                            ...
                            if new_action != []:
                                ...
                                _sat_verify(instanitated_rule_rep, atom_current_state_rep, new_action_rep, "action_enableness_analysis")
    if enabled_actions == []:
            ...
            for rule in instanitated_rule:
                ...
                if en_Act != []:
                    if en_Act[0] not in enabled_actions:
                        ...
                        _sat_verify(instanitated_rule_rep, atom_current_state_rep, enabled_actions_rep, "action_enableness_analysis")
...
def communication_analysis(current_agent, all_agents, sent_message_update, action_constraints, domain,
    ...
    if action_constraints != []:
        for constraint_copy in instantiated_constraints:
            constraint = _fast_deepcopy(constraint_copy)
            Cons = existential_variable_rule_instantiation(constraint, action_constraints, constants)
            for cons in Cons:
                new_cons = pattern_match_at_left_rule(cons, action_constraints)
                ...
                if generated_info != []:
                    ...
                    if agent_info == 'all':
                        ...
                        received_agents = _fast_deepcopy(all_agents)
...
def enabled_events_generation(enabled,event_updates,atoms_current_state_copy,atoms_goal_state_copy,constants):
    if enabled[0] == "insert":
        ...
        if enabled[1] not in atoms_goal_state_copy and enabled[1] not in atoms_current_state_copy:
            ...
            added_current_atom=_fast_deepcopy(enabled[1])
        ...
        if enabled[1] in atoms_goal_state_copy:# and enabled[1] in atoms_current_state_copy:
            ...
            for atom in atoms_current_state_copy:
                if atom['name'][0:7]!='a-goal-':
                    A.append(atom)
            atoms_current_state_copy=_fast_deepcopy(A)
...
def event_analysis(received_messages, event_processing, atoms_current_state, atoms_goal_state, domain, constants):
    ...
    enableness_rule = []
    atoms_current_state_copy = _fast_deepcopy(atoms_current_state)
    atoms_goal_state_copy = _fast_deepcopy(atoms_goal_state)
    ...
    for rule1 in instantiated_constraints:
        if rule1 in communication_processing:
            if received_messages != []:
                ...
                rule_copy = _fast_deepcopy(rule1)
                p1 = rule_copy[0]
                p2 = rule_copy[1]
                j = 0
                        ...
                        for rule in fully_instantiated:
                            ...
                            if msg in received_messages:
                                ...
                                while i < len(rule):
                                    ...
                                    i = i + 1
                                new_rule = pattern_match_at_left_rule(new_rule, atoms_current_state_copy)
                                enabled = derivation_at_right_rule(new_rule)
                                if enabled != []:
        ...
        if rule1 in non_communication_processing:
            ...
            for rule in fully_instantiated:
                rule_rep = rule_normal_representation(rule, constants)
                rule = pattern_match_at_left_rule(rule, atoms_current_state_copy)
...
def state_transformer(enable_actions, current_state, knowledge_base, action_specification, domain, constants):
    ...
    next_state_beliefs = []
    for rule in instantiated_constraints:
        new_beliefs = _fast_deepcopy(current_state)
        pos = find_position_in_list(rule, 'implies')[0]
        remove_predicates = []
        add_predicates = []
        previous_not = False
        current_enabled_action = None
        for predicate in rule[0:pos]:
            if predicate not in keywords:
                if predicate["name"] in list(action_specification.keys()):
                    current_enabled_action = predicate
                    break
        # No action predicate in this effect rule's body: nothing to instantiate
        # against the enabled actions, so skip it (guards against a NameError on
        # current_enabled_action; does not occur for well-formed effect rules).
        if current_enabled_action is None:
            continue
        ...
        if substitution != []:
            ...
            for sub in substitution:
                ...
                for rule2 in instantiated_rules:
                    remove_predicates_copy = _fast_deepcopy(remove_predicates)
                    add_predicates_copy = _fast_deepcopy(add_predicates)
                    ...
                    for predicate in rule2[pos + 1:]:
                        if predicate not in keywords:
                            ...
                            previous_not = False
                    new_rule = pattern_match_at_left_rule(rule2, current_beliefs)
                    if derivation_at_right_rule(new_rule) != []:
...
def DM_evaluation(agents, knowledge_base,  domain, constants, dummy_agents, prior_beliefs):
    ...
    for agent in agents:
        ...
        B = process_beliefs(agent.belief_base, constants)
...
def DM_generation(agents, knowledge_base, constraints_of_action_generation,enableness_of_actions, action_specification, sent_message_update, event_processing, domain, constants, dummy_agents, safety,  prior_beliefs):
    ...
    print("The evaluation flag is", evaluation_flag)
    while non_termination and loop_count < loop_max:
        ...
        for agent in agents:
            ...
            if not dummy_flag:
                ...
                if current_focus != []:
                    current_focus_copy = _fast_deepcopy(current_focus)
        ...
        for agent in agents:
            ...
            # Agents will be set to inactive at the current reasoning cycle when the following conditions simultaneously satisfy:
            # 1. An agent does not have any goals to achieve.
            # 2. A non-dummy agent encounters a fatal error.
            ...
            atom_current_desired = _fast_deepcopy(atom_current)
            ...
            if dropped_goals != []:
                substate_goal_copy = _fast_deepcopy(substate_dict[name][1][0])
        ...
        if (not empty_dict(next_move_dict) or not flag_task or active_agents == []) and loop_count > 3:
        ...
        if name not in dummy_agents:
            if name in next_move_dict.keys():
                ...
                if active_flag_dict[name]:
                    agent.belief_base = substate_dict_rep[agent.name][0]
                    agent.goals = substate_dict_rep[agent.name][1]
                    next_state = process_beliefs(agent.belief_base, constants)
                    NS = [x for x in next_state if x['name'] != 'released']
                    agent.desired_next_state = state_normal_representation(NS)
                else:
                    next_state = process_beliefs(agent.desired_next_state, constants)
                    NS = [x for x in next_state if x['name'] != 'released']
                    agent.desired_next_state = state_normal_representation(NS)
        else:
            agent.belief_base = substate_dict_rep[agent.name][0]
            #agent.belief_base = _fast_deepcopy(agent.desired_next_state)
            agent.desired_next_state = []
...
def update_common_beliefs(prior_beliefs, beliefs, constants):
    ...
    prior_beliefs = process_beliefs(prior_beliefs, constants)
    for b in beliefs:
        ...
        if predicate_name == 'on':
            ...
            # A bare sensor 'holding' carries no object argument, so it cannot
            # identify which on(object, location) to retract; guard the index
            # access to avoid an IndexError. (Known limitation: with a bare
            # 'holding' the stale on(...) fact is left in the shared world model.)
            if processed_b["values_in_non_list"]:
                Old_B = _fast_deepcopy(prior_beliefs)
                for item in Old_B:
                    if item['name'] == 'on' and item["values_in_non_list"]:
...
def interpreter(agents, knowledge_base, constraints_of_action_generation,
                 ...
                 prior_beliefs, sensor_files=None):
    # sensor_files (optional): {agent_name: "trajectory_file.txt"}. When given,
    # each non-dummy agent reads its own named sensor-trace file instead of the
    # built-in MG_0<t>.txt files, so a self-contained demo can ship its own
    # trajectories without clobbering the shared MG_0*.txt used by other specs.
    _clear_memoization_caches()  # fresh caches per run (domain/constants may differ between specs)
    global _SKIP_SAT
    _SKIP_SAT = sensor_files is not None  # story/demo runs skip the (slow) SAT verification
    agent_sensor_info_dict = {}
    if sensor_files is not None:
        for agent in agents:
            if agent.name not in dummy_agents:
                agent_sensor_info_dict.update({agent.name: info_parse_file(sensor_files[agent.name])})
    else:
    ...
    last_location_dict = {}
    delivery_error_dict = {}
    # Object each agent is physically carrying (set from its pickup(w) decision).
    # Used to keep the shared world model (prior_beliefs) in step with the sensor:
    # the offline sensor reports only Holding True/False, not which object, so we
    # remove on(w,*) when it picks up and add on(w,pos) when it drops.
    carried_object_dict = {}
    for agent in agents:
        if agent.name not in dummy_agents:
            ...
            carried_object_dict.update({agent.name: None})

    N = 0
    for key in agent_sensor_info_dict:
        if len(agent_sensor_info_dict[key]) > N:
            N = len(agent_sensor_info_dict[key])
    # Reactive sensor (sensor_files mode only): each agent advances through its own
    # trajectory only while it is still *executing* a commanded action; once its
    # sensed state reaches the action's target it HOLDS (stays ready) until the
    # next synchronised decision. This mirrors the real robot (the trace advances
    # because the robot moves on command) and keeps agents in step despite
    # different-length actions. Non-story runs (shared MG_0*.txt) are unchanged.
    reactive = sensor_files is not None
    sensor_ptr = {}
    cur_state = {}
    for agent in agents:
        if agent.name not in dummy_agents:
            sensor_ptr[agent.name] = 0
            cur_state[agent.name] = set()
    # The online task planner will continously generate safe decisions for the multi-agent system until no goals.
    generated_actions={}
    decision_count = 0
    while flag_task and i < (N * 2 if reactive else N):
        start=time.time()
        decision_requirement=False
        for agent in agents:
            if agent.name not in dummy_agents:
                sensor_info_list = agent_sensor_info_dict[agent.name]
                idx = sensor_ptr[agent.name] if reactive else i
                if idx >= len(sensor_info_list):
                    info_dict = sensor_info_list[-1]
                else:
                    info_dict = sensor_info_list[idx]
                ...
                translation_result=translation(info_dict,  old_last_holding,
                                                                    old_last_location)
                ...
                if reactive:
                    _cs = {'at(' + last_location + ')'}
                    if info_dict['Docked'] == 'True':
                        _cs.add('docked(' + last_location + ')')
                    if last_holding:
                        _cs.add('holding')
                    cur_state[agent.name] = _cs
                holding_change = not (last_holding == old_last_holding)
                if holding_change:
                    carried = carried_object_dict.get(agent.name)
                    if last_holding and carried is not None:
                        # picked the object up: it leaves its shelf in the world model
                        prior_beliefs = [b for b in prior_beliefs
                                         if not (isinstance(b, str) and b.startswith('on(' + carried + ','))]
                    elif (not last_holding) and carried is not None:
                        # dropped the object at the current location: record it there
                        new_on = 'on(' + carried + ',' + last_location + ')'
                        if new_on not in prior_beliefs:
                            prior_beliefs = prior_beliefs + [new_on]
                        carried_object_dict[agent.name] = None
        ...
        if decision_requirement:
            ...
            # Remember which object each agent picks, so the world-model update on
            # the (later) holding change knows which on(w,pos) to move.
            for aname in generated_actions:
                act = generated_actions[aname]
                if isinstance(act, str) and act.startswith('pickup(') and ')' in act:
                    carried_object_dict[aname] = act[act.index('(') + 1:act.index(')')]
        ...
        # Direction 1 (story/reactive mode): once an agent has completed ALL its
        # goals it releases the location reservations it still holds in the dummy
        # manager C (reserved(agent,loc) -> idle(loc)), so a settled agent no longer
        # blocks others from acquiring that spot. (A settled agent otherwise never
        # moves, and release is movement-triggered, so it would hold the spot forever.)
        if reactive:
            for ag in agents:
                if ag.name not in dummy_agents and ag.goals == []:
                    for dummy in agents:
                        if dummy.name in dummy_agents and isinstance(dummy.belief_base, list):
                            nb = []
                            for b in dummy.belief_base:
                                if isinstance(b, str) and b.startswith('reserved(' + ag.name + ','):
                                    idl = 'idle(' + b[b.index(',') + 1:b.index(')')] + ')'
                                    if idl not in nb:
                                        nb.append(idl)
                                else:
                                    nb.append(b)
                            dummy.belief_base = nb

        # Reactive advance: move each agent one step along its trace only while it
        # has NOT yet reached its commanded target; hold once it has (stay ready).
        if reactive:
            for agent in agents:
                if agent.name not in dummy_agents:
                    desired = set(x for x in agent.desired_next_state
                                  if x.startswith('at(') or x.startswith('docked(') or x == 'holding')
                    reached = bool(desired) and cur_state[agent.name] == desired
                    if not reached and sensor_ptr[agent.name] < len(agent_sensor_info_dict[agent.name]) - 1:
                        sensor_ptr[agent.name] += 1
...
def info_parse(test):
    return info_parse_file("MG_0" + str(test) + ".txt")


def info_parse_file(file_name):
    # Parse a sensor-trace file (one '{...}' dict per line) into a list of frames.
    sensor_info_list = []
    f = open(file_name, "r")
